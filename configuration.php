 ...
 if ($_POST["conf"]=='2')
	{
	 ...
	 if ($_POST[$pt4]=='on') $emul=1;
	 $debug=$_POST["debug"];
	 $query = 'UPDATE dev_dk SET formxml='.$formxml.', crq_enabl='.$crqenable.', emul='.$emul.', log='.$debug;
	 //echo $query.'<br>';
	 mysql_query ($query,$i);
	}
 if ($_POST["conf"]=='3')
...
?>
	...
	<td class="centercol" style="width:630px">
		...
		<?php
		 ...
		 while ($ur = mysql_fetch_row ($r)) 
		    { 
		     $idtype=$ur[4]; $device=$ur[5]; $lib=$ur[7];
		     $query = 'SELECT COUNT(id) FROM device WHERE type='.($ur[4]);
		     if ($r2 = mysql_query ($query,$i))
		     if ($ur2 = mysql_fetch_row ($r2)) 
		        { 
		         $qnt=$ur2[0]; 
		         if ($qnt>0)
		            { 		
			     $query = 'SELECT * FROM threads WHERE thread='.$idtype;
	    		     ...
	    		     if ($uy = mysql_fetch_row ($a))
				{
			        ...
			        {
			         $query = 'INSERT INTO threads SET thread='.$idtype.',start=1,status=1';
			         mysql_query ($query,$i);
				} 
			    }
			}
		 ...
		 while ($uy = mysql_fetch_row ($a))
			{
			 $query = 'SELECT * FROM devicetype WHERE ids='.($uy[1]);
			 if ($r = mysql_query ($query,$i))
			 if ($ur = mysql_fetch_row ($r)) { $idtype=$ur[4]; $device=$ur[5]; $lib=$ur[7]; } 
			 $query = 'SELECT COUNT(id) FROM device WHERE type='.($uy[1]);
			 if ($r = mysql_query ($query,$i))
			 ...
			 if ($lib!='')
				...
				// print '<td class="ds_last qb_line" align="left" nowrap="nowrap">'.$uy[7].'</td>';
				 print '<td class="ds_last qb_line" align="left" nowrap="nowrap"></td>';
	...
	<td class="rightcol" style="width:300px;">
		...
		<tr class=""><td class="ds_symbol qb_shad" align="left" nowrap="nowrap">&nbsp;request pause</td>
		<?php
    			print '<td><input type="checkbox" id="emul" name="emul" ';
